...
class ProjectPublicCatalogSeeder extends Seeder
{
    public function run(): void
    {
        ...
        $edificaciones = [
            'A.020 VIVIENDA' => [
                ...
                'Edificación para grupos de individuos' => [
        ...
        foreach ($edificaciones as $type => $classifications) {
            foreach ($classifications as $classification => $categories) {
                if (empty($categories)) {
                    $addRow('Edificaciones', $type, $classification, null);
                    continue;
                }
                foreach ($categories as $category) {
                    $addRow('Edificaciones', $type, $classification, $category);
                }
            }
        }
        ...
        foreach ($habilitaciones as $type => $classifications) {
            foreach ($classifications as $classification) {
                $addRow('Habilitaciones', $type, $classification, null);
            }
        }

        DB::table('project_public_catalog')->truncate();
        DB::table('project_public_catalog')->insert($rows);
    }
}
